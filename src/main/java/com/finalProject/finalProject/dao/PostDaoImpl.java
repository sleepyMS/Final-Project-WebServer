...
public class PostDaoImpl implements PostDao {
    ...
    public PostDaoImpl() {
        ...
        db3.add(new PostDto(3, "ENFJ밍식", "나는 계획따위 없어 무계획이야", "너는 다 계획이 있니? 나는 없어ㅎㅎ", 0, 0, "J 게시판"));
        db3.add(new PostDto(4, "ISTJ주녕", "오늘은 눈에 보이는 친구들 괴롭혀야지~", "항상 즉흥적으로 괴롭히는건 즐거워!", 0, 2, "J 게시판"));
        db3.add(new PostDto(8, "ISFJ정오", "나는 지금을 항상 생각해!", "항상 어떤일을 할때 계획을 세워 난. 너희는 어떠니?", 0, 1, "J 게시판"));
        db.put("J 게시판", db3);

        // N와 J 게시판 데이터 (카테고리 4)
        List<PostDto> db4 = new ArrayList<>();
        db4.add(new PostDto(6, "ISFJ정오", "내가 만약 좀비가 된다면???", "내가 좀비가 된다면 당장 친구들부터 잡아먹어야지", 0, 1, "N와 S 게시판"));
        db4.add(new PostDto(11, "ISFJ정오", "취업예정입니다. 어떤 공부해야할까요?", "우선 자격증과 포트폴리오부터 시작해보려고 합니다. 조언 댓글로 부탁해요~", 0, 1, "N와 S 게시판"));
